...
namespace Org.BouncyCastle.Math.EC.Rfc8032
{
    public abstract class Ed448
    {
        ...
        private static void ImplSignWithoutNonce(IXof d, byte[] h, byte[] s, byte[] pk, int pkOff, byte[] ctx, byte phflag,
                byte[] m, int mOff, int mLen, byte[] sig, int sigOff)
        {
            Dom4(d, phflag, ctx);
            d.BlockUpdate(h, ScalarBytes, ScalarBytes);
        ...
        private static void ImplSignWithNonce(IXof d, byte[] h, byte[] s, byte[] pk, int pkOff, byte[] ctx, byte phflag,
                byte[] m, int mOff, int mLen, byte[] sig, int sigOff)
        {
            Dom4(d, phflag, ctx);
            d.BlockUpdate(s, 0, ScalarBytes);
            d.BlockUpdate(m, mOff, mLen);
            d.DoFinal(h, 0, h.Length);
            ...
            byte[] m, int mOff, int mLen, byte[] sig, int sigOff)
        {
            ...
            {
                PruneScalar(sk, 0, s);
                ImplSignWithNonce(d, h, s, pk, pkOff, ctx, phflag, m, mOff, mLen, sig, sigOff);
            }
            else
            {
                PruneScalar(h, 0, s);
                ImplSignWithoutNonce(d, h, s, pk, pkOff, ctx, phflag, m, mOff, mLen, sig, sigOff);
            }
        }
