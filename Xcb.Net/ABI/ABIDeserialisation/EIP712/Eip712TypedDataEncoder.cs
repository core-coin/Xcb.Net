...
namespace Xcb.Net.ABI.EIP712
{
    public class Eip712TypedDataEncoder
    {
        ...
        public byte[] EncodeAndHashTypedData<T, TDomain>(T message, TypedData<TDomain> typedData)
        {
            ...
            return Sha3NIST.Current.CalculateHash(encodedData);
        }
        ...
        private byte[] HashStruct(IDictionary<string, MemberDescription[]> types, string primaryType, IEnumerable<MemberValue> message)
        {
            ...
            using (var writer = new BinaryWriter(memoryStream))
            {
                ...
                var typeHash = Sha3NIST.Current.CalculateHash(Encoding.UTF8.GetBytes(encodedType));
                writer.Write(typeHash);

                EncodeData(writer, types, message);

                writer.Flush();
                return Sha3NIST.Current.CalculateHash(memoryStream.ToArray());
            }
        }
                    ...
                    case "string":
                        {
                            ...
                            var abiValueEncoded = Sha3NIST.Current.CalculateHash(value);
                    ...
                    default:
                        {
                            if (memberValue.TypeName.Contains("["))
                            {
                                ...
                                using (var writerItem = new BinaryWriter(memoryStream))
                                {
                                    ...
                                    writerItem.Flush();
                                    writer.Write(Sha3NIST.Current.CalculateHash(memoryStream.ToArray()));
                                }
